...
window.addEventListener("load", () => {
	...
	// Accordion
	const dynamicaContent = function (e) {
	...
	const accordionFunc = () => {
		...
		let lastAccordion = accordion[0];

		for (let i = 0; i < accordion.length; i++) {
			accordion[i].addEventListener("click", function () {
				if (lastAccordion === this) {
					this.classList.toggle("active");
					dynamicaContent(this);
					console.log("1");
				} else {
					console.log("2");
					lastAccordion.classList.remove("active");
					this.classList.toggle("active");
					dynamicaContent(this);
					dynamicaContent(lastAccordion);
					lastAccordion = this;
				}
			});

			dynamicaContent(accordion[i]);
		}
	};
	accordionFunc();
});
